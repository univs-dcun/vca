    ...
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }}
      ...
      <div style={{ backgroundColor: "white", borderRadius: "16px", border: BORDER, maxWidth: "440px", width: "100%", boxShadow: "0 20px 60px rgba(14,22,42,0.18)" }}>
        <div style={{ padding: "16px 20px", borderBottom: BORDER, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <p style={{ fontSize: "16px", fontWeight: 800, color: "#0e162a" }}>Register VIP</p>
        ...
        <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "14px" }}>
          <div>
            <label style={{ fontSize: "12px", fontWeight: 700, color: "#475469", display: "block", marginBottom: "6px" }}>Full Name *</label>
          ...
          <div>
            <label style={{ fontSize: "12px", fontWeight: 700, color: "#475469", display: "block", marginBottom: "6px" }}>Description</label>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "20px" }}>
        <div>
          <p style={{ fontSize: "16px", fontWeight: 800, color: "#0e162a" }}>VIP Registry</p>
          <p style={{ fontSize: "12px", color: "#64748a", marginTop: "4px" }}>Register watchlist individuals for this project&apos;s cameras to detect.</p>
        </div>
      ...
      {projectPersons.length === 0 ? (
        <div style={{ backgroundColor: "white", border: BORDER, borderRadius: "12px", padding: "32px", textAlign: "center" }}>
          <p style={{ fontSize: "13px", color: "#94a3b8" }}>No VIPs registered for this project yet.</p>
        </div>
      ) : (
          ...
          {projectPersons.map(person => (
            <div key={person.id} style={{ backgroundColor: "white", border: BORDER, borderRadius: "12px", boxShadow: PANEL_SHADOW, padding: "14px", display: "flex", gap: "12px", alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: "13px", fontWeight: 700, color: "#0e162a" }}>{person.name}</p>
                {person.description && <p style={{ fontSize: "10px", color: "#64748a", marginTop: "2px" }}>{person.description}</p>}
                <p style={{ fontSize: "10px", color: "#94a3b8", marginTop: "4px" }}>Registered {person.registeredAt}</p>
              </div>
              <button onClick={() => removePerson(person.id)} title="Remove VIP" style={{ border: "none", background: "none", cursor: "pointer", color: "#94a3b8", display: "flex", padding: "4px", flexShrink: 0 }}>
