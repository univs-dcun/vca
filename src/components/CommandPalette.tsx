...
function CameraGlyph() {
  return (
    <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
      ...
      <path d="M14.255 7.54373C14.4525 7.6426 14.6027 7.81584 14.6726 8.02539C14.7424 8.23493 14.7262 8.46363 14.6275 8.66123L12.0392 13.8371C11.9902 13.935 11.9225 14.0223 11.8398 14.094C11.7571 14.1657 11.661 14.2204 11.5572 14.255C11.4533 14.2896 11.3437 14.3034 11.2345 14.2956C11.1253 14.2878 11.0187 14.2586 10.9209 14.2096L3.00836 10.2496C2.43364 9.96007 1.99699 9.45471 1.79396 8.84407C1.59093 8.23342 1.63806 7.56722 1.92503 6.99123L3.07503 4.66623C3.21836 4.38058 3.41656 4.12597 3.65831 3.91693C3.90006 3.70788 4.18061 3.54851 4.48396 3.44791C4.78731 3.34731 5.1075 3.30746 5.42625 3.33062C5.74501 3.35378 6.05608 3.4395 6.34169 3.5829L14.255 7.54373Z" stroke="#94a3b8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M1.66663 15.8333H4.79996C5.11057 15.8355 5.4156 15.7508 5.68064 15.5888C5.94568 15.4269 6.16019 15.1941 6.29996 14.9167L7.49996 12.5" stroke="#94a3b8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M1.66675 17.4993V14.166" stroke="#94a3b8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      <path d="M5.83337 7.5H5.84067" stroke="#94a3b8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
}
...
export default function CommandPalette({ open, onClose, onGoToPage }: {
  ...
  const items = useMemo<PaletteItem[]>(() => [
    ...DATA_SUB_TABS.map(tab => ({
      group: "Data", label: tab, icon: <span style={{ color: "#94a3b8", display: "flex" }}>{TAB_ICONS[tab]}</span>, keywords: `data ${tab}`,
      action: () => goToData({ tab }),
    })),
    ...cameras.map(c => ({
  ...
  return (
    ...
    >
      <div style={{
        ...
        boxShadow: "0 24px 64px rgba(14,22,42,0.28)", border: "1px solid #e2e8f0",
        display: "flex", flexDirection: "column", overflow: "hidden",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "14px 18px", borderBottom: "1px solid #e2e8f0", flexShrink: 0 }}>
          <span style={{ color: "#94a3b8", display: "flex" }}><SearchGlyph /></span>
          <input
            ref={inputRef}
            value={query}
            ...
            style={{ flex: 1, border: "none", outline: "none", fontSize: "14px", color: "#0e162a", fontWeight: 600 }}
          />
          <span style={{ fontSize: "12px", fontWeight: 700, color: "#94a3b8", backgroundColor: "#f1f5f9", padding: "2px 6px", borderRadius: "5px" }}>ESC</span>
        </div>

        {dataGroup && (
          <div style={{ display: "flex", gap: "8px", padding: "10px 10px 4px", flexWrap: "wrap", borderBottom: "1px solid #e2e8f0" }}>
            {dataGroup.items.map(item => {
              ...
              return (
                ...
                >
                  <span style={{ display: "flex", color: active ? "#5a3dfb" : "#94a3b8" }}>{item.icon}</span>
                  <span style={{ fontSize: "12px", fontWeight: 600, color: active ? "#5a3dfb" : "#0e162a", whiteSpace: "nowrap" }}>{item.label}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="vca-hide-scrollbar" style={{ flex: 1, overflowY: "auto", padding: "8px" }}>
          {filtered.length === 0 && (
            <div style={{ padding: "32px 0", textAlign: "center", fontSize: "13px", fontWeight: 600, color: "#94a3b8" }}>
              No matches for &quot;{query}&quot;.
            </div>
          )}
          {listGroups.map(({ group, items: groupItems }) => (
            <div key={group} style={{ marginBottom: "6px" }}>
              <div style={{ padding: "8px 10px 4px", fontSize: "10px", fontWeight: 600, color: "#94a3b8", letterSpacing: "0.02em", textTransform: "uppercase" }}>
                {group}
              </div>
              {groupItems.map(item => {
                const flatIndex = filtered.indexOf(item);
                ...
                return (
                  ...
                  >
                    {item.avatar ? (
                      ...
                      <span style={{ width: "24px", height: "24px", borderRadius: "6px", backgroundColor: "#f1f5f9", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {item.icon}
                      </span>
                    )}
                    <span style={{ fontSize: "13px", fontWeight: 600, color: active ? "#5a3dfb" : "#0e162a" }}>{item.label}</span>
                    {item.sublabel && (
                      <span style={{ fontSize: "12px", color: "#94a3b8", marginLeft: "auto" }}>{item.sublabel}</span>
                    )}
                  </button>
                );
              })}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "14px", padding: "8px 16px", borderTop: "1px solid #e2e8f0", flexShrink: 0 }}>
          <span style={{ fontSize: "10px", fontWeight: 600, color: "#94a3b8" }}>↑↓ Navigate</span>
          <span style={{ fontSize: "10px", fontWeight: 600, color: "#94a3b8" }}>↵ Select</span>
          <span style={{ fontSize: "10px", fontWeight: 600, color: "#94a3b8", marginLeft: "auto" }}>⌘K to toggle</span>
        </div>
      </div>
    </div>
  );
