 ...
 */
import type { PortalConnectivityCamerasItem } from './portalConnectivityCamerasItem';

export interface PortalConnectivity {
  total: number;
  online: number;
  offline: number;
  /** 예약 — 현 모듈 계약은 RUNNING/STOPPED 2값 */
  error: number;
  /** status 미수신(모듈 미기동·EMQX 미접속) */
  unknown: number;
  cameras: PortalConnectivityCamerasItem[];
}
