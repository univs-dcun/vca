...
/**
 * 베스트 프레임 이력 1장 (계약 v1.5). imageUrl은 /api 상대경로 — <img src>로 직접 사용
 */
export interface BestFrameHistoryItem {
  /** 프레임 대상 조회 키 (…/bestframes/{frameId}/targets) */
  frameId: string;
  capturedAt: string;
  imageUrl: string;
  /**
   * 프레임 내 tracked 대상 수 — 썸네일 배지 표시용
   * @minimum 0
   */
  targetCount: number;
  /** 프레임에 존재하는 카테고리 (중복 제거) — 타임라인 카테고리 레인 표시용 */
