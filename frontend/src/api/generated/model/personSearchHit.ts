/**
 * Generated by orval v7.21.0 🍺
 * Do not edit manually.
 * VCA API
 * 백엔드가 생성/관리하는 API 계약. 프론트는 orval(npm run gen:api)로 이 스펙에서 타입/훅을 생성한다.

실시간 데이터(카메라 상태 변화, VIP 감지 이벤트, 집계 카운터)는 이 API가 아닌 MQTT로 전달된다 — 채널 경계와 병합 규칙은 vca-mqtt-broker 레포의 SPEC.md 참고. REST는 스냅샷(페이징 목록·이력·그래프)을, MQTT는 그 이후의 변화분을 담당한다.

공통 규약:
- 모든 응답은 { success, data, message, code } envelope
- 페이징은 { content, page, size, totalElements } (page는 0부터)
- 시각은 ISO-8601 UTC, 날짜 파라미터의 기본값은 사이트 로컬(Asia/Singapore) 기준 오늘
- ID는 문자열: cameraId/locationId는 ^[a-z0-9-]{1,64}$ (MQTT 토픽 경로와 공유)
- similarity는 0~1 실수 (표시 포맷은 프론트 책임)
 * OpenAPI spec version: 0.5.0
 */
import type { LatLng } from './latLng';
import type { PersonSearchHitMatchedVip } from './personSearchHitMatchedVip';

/**
 * 인물 검색 목격 1건 (계약 v1.2, UV-34). faceUrl/bodyUrl은 /api 상대경로 — <img src>로 직접 사용. faceScore/bodyScore는 해당 이미지를 검색 조건으로 제공했을 때만 채워진다
 */
export interface PersonSearchHit {
  /** 검색 결과 내 고유 ID — 크롭 이미지 조회 키 */
  hitId: string;
  /** @pattern ^[a-z0-9-]{1,64}$ */
  cameraId: string;
  cameraName: string;
  /** @pattern ^[a-z0-9-]{1,64}$ */
  locationId: string;
  location: LatLng;
  capturedAt: string;
  /**
   * 얼굴 유사도. face 이미지 미제공 또는 hit에서 얼굴 미검출이면 null
   * @minimum 0
   * @maximum 1
   * @nullable
   */
  faceScore: number | null;
  /**
   * 바디 유사도. body 이미지 미제공이면 null
   * @minimum 0
   * @maximum 1
   * @nullable
   */
  bodyScore: number | null;
  /**
   * 등록 인물 DB 매칭 정보 — null이면 미등록 인물 (화면이 경고 마커로 표시)
   * @nullable
   */
  matchedVip: PersonSearchHitMatchedVip;
  /**
   * 얼굴 크롭 이미지 — GET /search-hits/{hitId}/face. 얼굴 미검출 hit이면 null
   * @nullable
   */
  faceUrl: string | null;
  /**
   * 바디 크롭 이미지 — GET /search-hits/{hitId}/body. 전신 미검출 hit이면 null
   * @nullable
   */
  bodyUrl: string | null;
}
