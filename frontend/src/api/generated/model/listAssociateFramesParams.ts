...
import type { ListAssociateFramesBucket } from './listAssociateFramesBucket';

export type ListAssociateFramesParams = {
/**
 * 페이지 번호 (0부터)
 * @minimum 0
 */
...
size?: SizeParameter;
/**
 * (선택) 이 장소의 프레임만 — totalElements는 필터 적용 후 건수. Peak location 카드 필터용이며, 값이 topLocation.locationId일 때 totalElements는 topLocation.count와 같아야 한다
 * @pattern ^[a-z0-9-]{1,64}$
 */
locationId?: string;
/**
 * (선택) 이 시간대(capturedAt의 사이트 로컬 기준)의 프레임만. Peak time 카드 필터용이며, 값이 pattern.peakPeriod.bucket일 때 totalElements는 peakPeriod.count와 같아야 한다
 */
bucket?: ListAssociateFramesBucket;
};
