...
/**
 * RedFace 동료 목록 요청 (계약 v1.8). 대상 참조는 v1.4와 동일(source + targetId=감지 eventId), 기간 생략 시 사이트 로컬 최근 7일
 */
export interface AssociatesRequest {
  source: TrackSource;
  /** primary target의 감지 eventId — Re-ID 검색 매치·Live Monitoring 카드가 가진 값 그대로 */
  targetId: string;
  /**
   * 집계 시작 (생략 시 최근 7일)
   * @nullable
