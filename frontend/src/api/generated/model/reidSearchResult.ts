 ...
 */
import type { ReidSearchResultApplied } from './reidSearchResultApplied';
import type { ReidMatch } from './reidMatch';

/**
 * Re-ID 검색 결과 (계약 v1.7). results는 유사도 내림차순 상위 최대 maxResults건 — 화면이 순서 그대로 렌더
 */
export interface ReidSearchResult {
  /** 검색 실행 ID — 로그 추적·향후 비동기 전환 대비 */
  searchId: string;
  /** 백엔드가 실제 적용한 검색 구간·임계값 에코 */
  applied: ReidSearchResultApplied;
  /** 유사도 내림차순, 최대 maxResults건 — 없으면 빈 배열 */
  results: ReidMatch[];
}
