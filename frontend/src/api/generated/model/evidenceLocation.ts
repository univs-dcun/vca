 ...
 */
import type { EvidenceLocationEvent } from './evidenceLocationEvent';

/**
 * 동반 감지 장소 그룹 — count 내림차순. events는 최신순 최대 5건 표본(전량 아님)
 */
export interface EvidenceLocation {
  /** @pattern ^[a-z0-9-]{1,64}$ */
  locationId: string;
  /** 장소 표시명 (해당 위치 카메라의 대표 이름) */
  label: string;
  /**
   * 이 장소의 동반 감지 총횟수 — 전 그룹 합계 = totalEvents
   * @minimum 1
   */
  count: number;
  /** 최신순 최대 5건 */
  events: EvidenceLocationEvent[];
}
