 ...
 */
import type { EvidencePatternTopLocation } from './evidencePatternTopLocation';
import type { EvidencePatternPeakPeriod } from './evidencePatternPeakPeriod';

/**
 * 동반 패턴 집계 — 화면이 'Mostly seen together at {topLocation.label} ({count}x), mainly during {peakPeriod.bucket} hours ({ratio}%)' 문구를 조립한다
 */
export interface EvidencePattern {
  topLocation: EvidencePatternTopLocation;
  peakPeriod: EvidencePatternPeakPeriod;
}
