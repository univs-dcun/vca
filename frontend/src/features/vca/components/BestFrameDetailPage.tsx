...
export interface DetailProps {
   ...
   *  없거나 백엔드 미응답이면 이 파일의 mock(균등 샘플 스트립/data.detections) 흐름 유지 */
...
function AlsoCapturedCard({ det, index }: { det: Detection; index: number }) {
  // 데이터 연결(UV-37): 라이브 감지는 실제 스냅샷 크롭(vca-bridge 공급), mock은 기존 사진
  const photo = det.snapshotUrl ?? (det.type === "Vehicle" ? VEHICLE_PHOTO : AVATAR[index % AVATAR.length]);
...
function AIInspectionDetail({ det, data, eventDate, onClose, onGoRedmapTrace }: { det: Detection; data: CamData; eventDate?: string; onClose: () => void; onGoRedmapTrace?: (name: string, det?: Detection) => void }) {
  ...
  return (
    <div style={{ width:"380px", flexShrink:0, backgroundColor:"white", borderLeft:BORDER, display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"14px" }}>
        ...
        <div style={{ display:"flex", alignItems:"flex-end", gap:"8px", marginBottom:"14px" }}>
          <div style={{ flex:"0 0 77px", display:"flex", flexDirection:"column", alignItems:"center", gap:"4px" }}>
            {/* 데이터 연결(UV-37): 라이브 감지는 실제 스냅샷 크롭, 없으면 mock */}
          ...
          <div style={{ flex:"0 0 176px", display:"flex", flexDirection:"column", alignItems:"center", gap:"4px" }}>
            {/* 데이터 연결(UV-37): 라이브 등록 인물(VIP·Staff)은 실제 등록 사진, 없으면 mock */}
        ...
        <div style={{ marginBottom:"14px" }}>
          ...
          {[["Camera name", data.location], ["Event time", `${eventDate ?? sgtDateKey(new Date())} ${det.time}`]].map(([k, v]) => (
      ...
      <div style={{ padding:"10px 14px", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px", flexShrink:0 }}>
        ...
        {/* 데이터 연결(UV-36/37): Track on Map에 감지(det)를 실어 보내 대상 참조를 만들 수 있게 한다 */}
        <button onClick={() => onGoRedmapTrace?.(det.name, det)} style={{ padding:"9px 0", borderRadius:"8px", border:"none", backgroundColor:"var(--gray-900)", color:"white", fontSize:"13px", fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"6px" }}>
...
/* ── Main component ──────────────────────────────────────────── */
export default function BestFrameDetailPage({ data, initialDet, onBack, onGoRedmapTrace, autoOpenDetail, initialDate, initialTime, analyzeSource }: DetailProps) {
  ...
  // 백엔드가 응답하면 썸네일 스트립·메인 프레임·Best Frame Reel이 분 단위 이력 데이터로
  // 전환되고, 미응답이면 이 파일의 mock(균등 샘플 스트립/data.detections) 흐름이 그대로 유지된다.
  ...
  // 현재 선택 시각(초, SGT) — 스텝 readout·hour/minute 바·Jump to의 공통 기준.
  // 라이브는 선택 프레임의 실제 시각(없으면 열려 있는 분 창의 시작), mock은 기존 selectedSec.
  const currentSec = tl.live
    ? (tl.frames[tl.selectedIdx]
        ? hhmmssToSec(tl.frames[tl.selectedIdx].time)
        : (tl.window ? tl.window.hour * 3600 + tl.window.minute * 60 : selectedSec))
    : selectedSec;

  const axisSpanSec = AXIS_SPAN_SEC;
  const axisStartSec = windowCenterSec - axisSpanSec / 2;
  const currentHour = Math.floor(currentSec / 3600) % HOURS_IN_DAY;
  const currentMinute = Math.floor((currentSec % 3600) / 60);
  ...
  // 데이터 연결(UV-37): 라이브는 목표 시각이 속한 분(minute) 창을 조회하고(jump),
  // 스텝은 tl.step(±초)이 분 경계를 넘으면 다음 분을 이어서 조회한다.
  const jumpTo = (sec: number) => {
    if (tl.live) {
      tl.setMinute(dateLabel, Math.floor(sec / 3600) % HOURS_IN_DAY, Math.floor((sec % 3600) / 60));
      return;
    }
    setSelectedSec(sec); setWindowCenterSec(sec);
  };
  const stepBy = (deltaSec: number) => {
    if (tl.live) { tl.step(deltaSec); return; }
    setSelectedSec(s => s + deltaSec); setWindowCenterSec(s => s + deltaSec);
  };
  ...
  // 데이터 연결(UV-37): 라이브는 초당 1프레임 규칙 — step(1)이 분 경계에서 다음 분을 이어서 조회한다.
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (tlLive) tlStep(1);
      else { setSelectedSec(s => s + THUMB_STEP_SEC); setWindowCenterSec(s => s + THUMB_STEP_SEC); }
    }, tlLive ? 1000 : 700);
  ...
  useEffect(() => {
    ...
    // tlLive: 라이브 전환 시 stepBy의 라이브 분기가 반영된 클로저로 재구독 (UV-37)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dateOpen, selectedPerson, tlLive]);

  // 스트립 선택 인덱스 — 라이브는 브리지의 선택 프레임, mock은 샘플 축에서 가장 가까운 칸
  const stripIdx = tl.live ? tl.selectedIdx : centerThumbIdx;
  ...
  useEffect(() => {
    thumbRefs.current[stripIdx]?.scrollIntoView({ inline:"center", block:"nearest", behavior:"smooth" });
  }, [stripIdx]);
    ...
    <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", backgroundColor:"white" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", position:"relative" }}>
        ...
        <div style={{ flex:1, position:"relative", display:"flex", alignItems:"center", justifyContent:"center", overflow:"auto", backgroundColor:"var(--gray-100)", minHeight:0, padding:"20px", boxSizing:"border-box" }}>
          ...
          >
            {/* 데이터 연결(UV-37): 라이브는 선택 프레임의 실제 이미지 */}
        ...
        <div style={{ backgroundColor:"white", borderTop:BORDER, flexShrink:0, display: (selectedPerson || data.sourceType === "image") ? "none" : "block" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"flex-end", gap:"10px" }}>
              ...
              <div ref={dateDropdownRef} style={{ position:"relative" }}>
                <button onClick={() => setDateOpen(o => !o)} style={{ display:"flex", alignItems:"center", gap:"6px", backgroundColor:"white", borderRadius:"8px", padding:"6px 12px", border:"1px solid var(--gray-300)", cursor:"pointer" }}>
                  ...
                  <span style={{ fontSize:"12px", fontWeight:700, color:"var(--gray-900)" }}>{dateLabel}</span>
                ...
                {dateOpen && (
                  <div style={{ position:"absolute", right:0, top:"calc(100% + 4px)", backgroundColor:"white", borderRadius:"8px", boxShadow:"0 4px 16px rgba(14, 22, 42,0.12)", border:"1px solid var(--gray-200)", overflow:"hidden", zIndex:50 }}>
                    {/* 데이터 연결(UV-37): 라이브는 같은 시/분을 유지한 채 날짜만 바꿔 재조회
                        (이력 보존 범위 밖 날짜는 미응답 — 기존 창 유지) */}
                    <TrackDateCalendar selected={dateLabel} onPick={(d) => {
                      setTrackDate(d);
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"6px", backgroundColor:"white", borderRadius:"8px", padding:"6px 12px", border:"1px solid var(--gray-300)" }}>
                ...
                <input
                  type="text" inputMode="numeric" placeholder="HH:MM:SS" defaultValue={secToHHMMSS(currentSec)}
                  onKeyDown={e => {
          ...
          <div style={{ padding:"8px 16px 0", position:"relative" }}>
            ...
            <div className="vca-thin-scrollbar" style={{ position:"relative", height:`${THUMB_STRIP_HEIGHT}px`, zIndex:2, display:"flex", alignItems:"flex-end", gap:"4px", overflowX:"auto" }}>
              {/* 데이터 연결(UV-37): 라이브는 분 이력의 실제 프레임(초당 1장·실이미지·실카운트),
                  mock은 기존 균등 샘플 합성 스트립 */}
              {(tl.live ? tl.frames.map(f => hhmmssToSec(f.time)) : thumbFrames).map((sec, i) => {
                const liveFrame = tl.live ? tl.frames[i] : null;
                  ...
                  <div key={liveFrame?.id ?? i} ref={el => { thumbRefs.current[i] = el; }} onClick={() => (tl.live ? tl.select(i) : setSelectedSec(sec))}
                    ...
                    <div style={{
                      ...
                      <div style={{ width:"100%", height:"100%", overflow:"hidden", borderRadius: isSelected ? "6px" : "7px", position:"relative", backgroundColor:"var(--gray-800)" }}>
                        <img src={liveFrame?.imageUrl ?? data.bgUrl ?? ""} alt=""
            ...
            <div style={{ position:"relative" }}>
              <div style={{ display:"flex", gap:"2px" }}>
                {Array.from({ length: HOURS_IN_DAY }, (_, h) => {
                  ...
                  return (
                    <button key={h} onClick={() => jumpTo(h * 3600 + (currentSec % 3600))}
            ...
            <div style={{ position:"relative", marginTop:"8px" }}>
              <div style={{ display:"flex", gap:"1px" }}>
                {Array.from({ length: MINUTES_IN_HOUR }, (_, m) => {
                  ...
                  return (
                    <button key={m} onClick={() => jumpTo(currentHour * 3600 + m * 60 + (currentSec % 60))}
