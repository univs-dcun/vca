...
function InviteUserModal({ defaultProjectId, onClose }: { defaultProjectId: string; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    // 데이터 연결(UV-52): 라이브면 계정 발급·초대 토큰이 서버에서 온다(Promise) — await로 양쪽 모두 처리
    const id = await addPortalUser({
    ...
    const token = await issueInviteToken(id);
...
function TempPasswordModal({ user, mailAvailable, initialCode, onClose }: { user: PortalUser; mailAvailable: boolean; initialCode?: string; onClose: () => void }) {
  ...
  const generate = async () => {
    ...
    const issued = await (kind === "password" ? issueTemporaryPassword(user.id) : issueSetupCode(user.id)); // 데이터 연결(UV-52)
    if (!issued) return;
    setPassword(issued);
    showToast({
      variant: "success",
          ...
          <div key={u.id} style={{
            ...
            <RowActionsMenu actions={[
              ...(mailAvailable && u.status === "invited" ? [{
                ...
                onClick: async () => showToast({ variant: "success", title: t.inviteResentTitle, desc: `/password-setup?token=${await issueInviteToken(u.id)}` }),
              }] : []),
              {
