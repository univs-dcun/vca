...
export default function PortalNewProjectWizard({ teamId, onDeployed, onCancel, defaultType = "smart_city" }: PortalNewProjectWizardProps) {
  ...
  const deploy = async () => {
    ...
    // 데이터 연결(UV-52): 라이브면 서버가 발급한 id가 돌아온다(Promise). mock은 void라 기존 diff 방식 유지
    const liveId = await addProject({ name: name.trim(), teamId, type: selectedType });
    const created = useVcaStore.getState().projects.find(p => !existingIds.has(p.id));
    // 데이터 연결(UV-52): 라이브면 서버 발급 id, mock이면 store diff로 찾은 행
    const projectId = liveId ?? created?.id ?? "";
    ...
    if (projectId && rtspUrl.trim()) {
      await addCamera({ // 데이터 연결(UV-52): 라이브면 POST /portal/cameras
        projectId,
