...
function ReIDContent({ camera, onCameraChange, seedCard, onSeedConsumed, onNavigateTab, onGoRedmap, onGoAnalyzeFrame }: {
  ...
  seedCard?: (typeof REID_DATA)[number] | null; onSeedConsumed?: () => void; onNavigateTab?: (tab: DataTab, card: (typeof REID_DATA)[number], label?: string) => void;
  ...
  const state: SearchFilterState = {
    ...
    // 데이터 연결(UV-39): 라이브 목록(vipList·recentList)에서 고른다 — mock 상수 직접 접근 대신
    activeTarget: activeVIP >= 0
      ? { face: vipList[activeVIP].face, label: vipList[activeVIP].name }
      : selectedTarget >= 0
        ? { face: recentList[selectedTarget].face, label: recentList[selectedTarget].label }
        : null,
  ...
  const liveCluster: ReidCluster | null = hasSearched && liveResults ? {
    ...
    // RedFace 인계용 인물(REID_DATA 모양) — 라이브 결과에는 없는 개념이라 검색 대상 행, 없으면 첫 mock
...
function PrimaryTargetPickerModal({ onConfirm, onCancel }:
  ...
  const candidates = liveCands ?? targetCandidates ?? candidatesFromFilters({ searchType, gender, apparel, props, dateRange, threshold, licensePlate, topColors, bottomColors, shoesColors, siteCameras });
  ...
  const autoSelectId = target && candidates.length > 0 ? candidates[0].id : null;
  ...
  // than falling back to the row's object id. 데이터 연결(UV-40): 라이브 후보면 매치의 감지 참조
  // (targetId/cameraId)가 행에 이미 실려 있어 그대로 동료 목록의 대상 참조가 된다.
...
function JointEvidencePanel({ primary, tier, node, onClose, onAnalyzeFrame, liveRef, dateRange }: {
      ...
      <div style={{ flexShrink:0, padding:"20px 20px 0", display:"flex", flexDirection:"column", gap:"18px" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"8px", margin:"2px 0",
          ...
          <div style={{ display:"flex", gap:"10px" }}>
            <button className="vca-peak-card" data-on={focus === "location"}
              ...
              title={t.peakLocationTip(topLocationCount, totalFrames)}
            ...
            <button className="vca-peak-card" data-on={focus === "time"}
              ...
              title={t.peakTimeTip(bucketCount, totalFrames, pct, attr(bucket, lang))}
              style={{ flex:1, minWidth:0, borderRadius:"8px", padding:"6px 10px", textAlign:"left", cursor:"pointer" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"6px", fontSize:"11px" }}>
            <span style={{ color:"var(--gray-400)", whiteSpace:"nowrap" }}>{t.first} <strong style={{ color:"var(--gray-900)", fontWeight:700 }}>{firstSeenLabel}</strong></span>
            ...
            <span style={{ flexShrink:0, fontSize:"9px", fontWeight:800, color:"var(--gray-700)", backgroundColor:"white",
              ...
              {t.framesCount(totalFrames)}
            </span>
            <span style={{ flex:1, minWidth:"12px", height:0, borderTop:"1px dashed var(--gray-300)" }} />
            <span style={{ color:"var(--gray-400)", whiteSpace:"nowrap" }}>{t.last} <strong style={{ color:"var(--gray-900)", fontWeight:700 }}>{lastSeenLabel}</strong></span>
          </div>
        </div>
        ...
        <div style={{ display:"flex", alignItems:"baseline", justifyContent:"space-between", gap:"8px" }}>
          ...
          <span style={{ display:"flex", alignItems:"center", gap:"6px", flexShrink:0 }}>
            ...
            <span style={{ fontSize:"10px", fontWeight:600, color:"var(--gray-400)", whiteSpace:"nowrap" }}>{t.pageRange(pageStart + 1, pageStart + pageRows.length, totalShown)}</span>
          </span>
        </div>
      </div>
        ...
        return (
          <div key={`${r.tier}-${r.node.id}`} style={{ backgroundColor: i === 0 ? "var(--primary-100)" : "white", borderTop:BORDER,
            ...
            <div style={{ flex:1, display:"flex", alignItems:"center", gap:"10px", minWidth:0 }}>
              ...
              <span style={{ fontSize:"13px", fontWeight:700, color:"var(--gray-900)", whiteSpace:"nowrap" }}>{r.node.label || t.associateNo(String(i+1).padStart(2,"0"))}</span>
            </div>
            ...
            <span style={{ width:"168px", flexShrink:0, fontSize:"12px", fontWeight:600, color:"var(--gray-500)", display:"flex", gap:"6px", whiteSpace:"nowrap" }}>
              ...
              {nowMs !== null && (
                <span style={{ color:"var(--gray-400)" }}>({t.agoOf(formatElapsed(nowMs - (r.node.lastSeen ? new Date(`${r.node.lastSeen.replace(" ", "T")}:00+08:00`).getTime() : parseSgtStamp(lastSeen.date, lastSeen.time).getTime()), lang))})</span>
              )}
            </span>
...
function AssociateGraphView({ primaryTarget, onSwitchTarget, onGoAnalyzeFrame, liveRef }: {
  ...
  const inDateRange = (n: RedfaceNode) => {
    ...
    return cooccurEventsInRange(n, cooccurCams, dateRange).length > 0;
  };
...
function RedFaceContent({ seedCard, seedLabel, onSeedConsumed, onGoAnalyzeFrame }: {
  ...
  if (seedCard !== prevSeedCard) {
    ...
    if (seedCard) {
      ...
      const label = live.label ?? seedLabel ?? (seedCard.status === "VIP" ? "VIP Match" : `TS${String(seedCard.id).padStart(6, "0")}`);
      setPrimaryTarget({
        name: label,
        face: seedCard.url,
