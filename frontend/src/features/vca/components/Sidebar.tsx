...
import {
  vcaEventsToLiveEvents, todaysDetectionHits,
...
function useEventCounts() {
  ...
  return {
    ...
    eventsToday: live?.eventsToday ?? { ...(dashboardStats?.eventsToday ?? flatDelta), count: todaysHitCount },
  };
}
...
function SystemTab({ onPinDevice, pinnedDeviceId: externalPinnedId }: SystemTabProps) {
  ...
  // Broker-fed camera list when live (MQTT status), the projected list otherwise.
