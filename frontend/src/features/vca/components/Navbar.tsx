...
export default function Navbar({ activeTab: externalTab, onTabChange, onNotificationSelect, sidebarPosition, onSidebarPositionChange }: NavbarProps) {
  ...
  // 데이터 연결: 브로커가 stats/summary를 발행하면 그 값이 우선, 아니면 기존 폴백.
  const live = useLiveDashboardStats();
  const camsRunning = live?.camsRunning ?? (dashboardStatsError ? "—" : dashboardStats?.linkedCams.count ?? 0);
  const camsStopped = live?.camsStopped ?? (dashboardStatsError ? "—" : dashboardStats?.offlineCams.count ?? 0);
