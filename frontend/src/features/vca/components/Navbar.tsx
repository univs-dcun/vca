...
export default function Navbar({ activeTab: externalTab, onTabChange, onNotificationSelect, sidebarPosition, onSidebarPositionChange }: NavbarProps) {
  ...
  // Running/Stopped come from the broker's stats/summary when live; the stub values otherwise.
  const live = useLiveDashboardStats();
  const aiRunning = live?.aiRunning ?? (dashboardStatsError ? "—" : dashboardStats?.aiRunning ?? 0);
  const aiStopped = live?.aiStopped ?? (dashboardStatsError ? "—" : dashboardStats?.aiStopped ?? 0);
  ...
  // Tracking 행도 포함 — 새 행 분류 규칙(반입 2026-08-27)에서는 VIP가 2대 이상 카메라에 잡히는
  // 순간 그 인물의 행 전체가 Tracking 1행으로 접힌다. 라이브에서는 대부분의 VIP가 금세 이 상태가
  // 되므로 personType === "VIP"만 보면 벨이 항상 비어 버린다 — 벨의 의미(VIP가 카메라에 감지되면
  // 알림)에는 두 행 타입 모두 VIP 인물의 감지다.
  const vipEvents = events.filter(e => (e.personType === "VIP" || e.personType === "Tracking")
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: "8px" }}>
                          ...
                          <span style={{ display: "flex", alignItems: "baseline", gap: "6px", minWidth: 0 }}>
                            ...
                            {/* Tracking 행(경로 집계)은 confidence를 0으로 두므로 0%는 표시하지 않는다 */}
                            {ev.confidence > 0 && <span style={{ fontSize: "11px", fontWeight: 700, color: "var(--primary-400)", flexShrink: 0 }}>{ev.confidence}%</span>}
                          </span>
            ...
            {settingsOpen && (
              <div style={{
                ...
                <button
                  ...
                  onClick={() => { setSettingsOpen(false); authLogout().then(() => router.push("/login?reason=signedOut")); }}
                  style={{
                    display: "flex", alignItems: "center", gap: "10px", width: "100%",
                    padding: "9px 8px", borderRadius: "10px", border: "none",
                    cursor: "pointer", textAlign: "left",
