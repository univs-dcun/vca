...
import { getAssociateEvidence, listTargetAssociates } from '../../api/generated/search/search'
  ...
  try {
    ...
    return {
      nodes: (data.associates ?? []).map((a) => ({
        ...
        topCameraLabel: `${a.topCamera.cameraName} (${a.topCamera.count}x)`,
...
export interface RedfaceEvidenceView {
  ...
  firstSeen: string
  lastSeen: string
  topLocation: { label: string; count: number }
...
export async function fetchRedfaceEvidence(ref: RedfacePrimaryRef, associateId: string): Promise<RedfaceEvidenceView | null> {
  try {
    ...
    return {
      ...
      topLocation: { label: data.pattern.topLocation.label, count: data.pattern.topLocation.count },
