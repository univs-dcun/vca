...
| 책임 | 상태 |
|---|---|
| 카메라 원장 CRUD (자격증명 AES-GCM 암호화 보관) | P1 (이 골격) |
| 원장 변경 시 모듈 provisioning — `PUT {module}/provision/cameras` (계약 v1.9, 전체 목록 선언적 멱등 교체) | P1 (이 골격) |
| 미디어 서버(MediaMTX) 스트림 경로 동기화 | P2 |
...
- 기동 시 원장이 비어 있으면 기본 카메라 8대를 시드하고(끄려면 `VCA_ADMIN_SEED=false`),
  기동마다 모듈에 provisioning을 1회 push한다 (모듈 재기동 후 수렴 보장).
- 모듈이 죽어 있어도 CRUD는 성공한다 — 원장이 원천이고, `POST /admin/api/provision/sync`로 따라잡는다.
...
| `POST /admin/api/provision/sync` | 모듈 provisioning 수동 재동기화 |
| `GET /admin/api/provision/status` | 마지막 동기화 상태 |
